"""
NSPT Qubit J Estimator — Online Belavkin Filter + Offline Newton Stack
=======================================================================

Two-layer architecture:

LAYER 1 — Online Belavkin Filter
─────────────────────────────────
  Input  : raw homodyne stream dy(t)
  State  : (x̂, ŷ, ẑ)  [3 floats = 12 B]
  Buffer : shift-register of length LAG  [LAG × 4 B ≈ 232 B]
  Output : C_target = online ⟨ẑ(t)·ẑ(t−τ₀)⟩  [1 float]
  Total hardware state ≈ 250 bytes

  The innovation  dW_est = dy − 2√(ηκ) ẑ dt  strips shot noise.
  C_filter(J*, τ₀) is monotone in J*, approaching 0 as J*→J_true.

LAYER 2 — Offline NSPT Newton
──────────────────────────────
  Input  : C_target, current J*
  Runs   : NSPT tower (0th + 1st order Bloch) with fresh model noise
  Output : J* ← J* + (C_target − C0) / C1

Outer loop (converges in ~10 passes from 60% initial error):
  The outer loop is a fixed-point iteration.
  Fixed point: C_filter(J*, τ₀) = C_model(J*, τ₀) = 0  ⟺  J* = J_true
  Each pass advances J* by ~0.15–0.25 toward J_true.
  Convergence is monotone because C_filter is monotone in J*.
"""
